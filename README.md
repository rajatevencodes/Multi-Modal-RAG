...
**Redis (Auto-configured in Docker-compose file)**
```bash
...
>
> Docker containers are **isolated** from your computer. Inside Docker, `localhost` means the container itself. To access services running on your machine (like Supabase), containers use `host.docker.internal` instead of `localhost`.
>
